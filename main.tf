...
resource "google_compute_instance" "minecraft" {
  ...
  zone                    = local.zone
  tags                    = ["minecraft"]
  metadata_startup_script = "docker run -d --name mcserver -p 42865:25565 -e EULA=TRUE -e VERSION=1.18.2 -e MEMORY=1.5G -e OPS=rinsuki,takanakahiko -v /var/minecraft:/data itzg/minecraft-server:latest;"
  metadata = {
    enable-oslogin  = "TRUE"
